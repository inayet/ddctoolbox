...
switch message action="switch":
    ...
    if [ -z "{{ message }}" ]; then
        ...
        if [ "{{ action }}" = "switch" ]; then
            echo "You picked the default action: switch"
        else
            echo "You picked action: {{ action }}"
        fi
...
#       # just r
#       # just gact "{{ message }}"
#       sudo nixos-rebuild --verbose --log-format bar-with-logs --show-trace --print-build-logs --json --flake .'#inixos' {{ action }}
#       #sudo nixos-rebuild --json --flake .'#inixos' test
#     fi
...
[group("Shell Aliases")]
u-1:
    sudo nix run .#nixosConfigurations.inixos.config.facter.debug.nix-diff
