...
VdcEditorWindow::VdcEditorWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::VdcEditorWindowHost),
      undoStack(new QUndoStack(this)), undoView(new QUndoView(undoStack)),
      bwCalc(new BwCalculator(this)) {
...
#define DECL_UNDO_ACTION(type)                                                 \
  ...
  action##type->setIcon(                                                       \
      QPixmap(":/img/" + QString(#type).toLower() + ".svg"));                  \
...
#undef DECL_UNDO_ACTION
  ...
  connect(&VdcProjectManager::instance(), &VdcProjectManager::projectClosed,
          undoStack, &QUndoStack::clear);
  connect(&VdcProjectManager::instance(), &VdcProjectManager::projectClosed,
          undoStack, [] { Biquad::ResetIds(); });
  connect(&VdcProjectManager::instance(),
          &VdcProjectManager::projectMetaChanged, [this] {
            ...
            QString fileName =
                QFileInfo(VdcProjectManager::instance().currentProject())
                    .fileName();

            title += " - " + fileName + "[*]";

            this->setWindowModified(
                VdcProjectManager::instance().hasUnsavedChanges());
...
// OS session management
  ...
  connect(qApp, &QGuiApplication::commitDataRequest, this,
          &VdcEditorWindow::commitData);
#endif

  setUnifiedTitleAndToolBarOnMac(true);

  // Restore settings
  ui->actionSwitch_orientation->setChecked(
      settings.value("verticalOrientation").toBool());
  ui->actionToggle_marker_points->setChecked(
      settings.value("pointsAlwaysVisible").toBool());
...
VdcEditorWindow::~VdcEditorWindow() { delete ui; }

/** -------- Initialization -------- **/

void VdcEditorWindow::createFilterModel() {
  filterModel = new FilterModel();
  connect(filterModel, &FilterModel::dataChanged,
          &VdcProjectManager::instance(), &VdcProjectManager::projectModified);
  connect(filterModel, &FilterModel::dataChanged, this,
          &VdcEditorWindow::drawPlots);
  connect(filterModel, &FilterModel::filterEdited,
          [this](DeflatedBiquad previous, DeflatedBiquad current,
                 QModelIndex index) {
            EditCommand *editCmd =
                new EditCommand(filterModel, previous, current, index);
  ...
  ui->tableView_DDCPoints->horizontalHeader()->setSectionResizeMode(
      QHeaderView::Stretch);
  connect(ui->tableView_DDCPoints->selectionModel(),
          &QItemSelectionModel::selectionChanged, [=] { updatePlots(true); });
}
...
void VdcEditorWindow::createUpdater() {
  ...
  updateLabel->setText("A new version of this application has been released. "
                       "Press 'Install' to update automatically.");

  ui->updateBar->setStyleSheet("QStatusBar {"
  ...
  connect(ui->actionCheck_for_updates, &QAction::triggered, swUpdater,
          &SoftwareUpdateManager::checkForUpdates);

  connect(notNowButton, &QAbstractButton::clicked,
          [this] { ui->updateBar->setVisible(false); });
  connect(changelogButton, &QAbstractButton::clicked, swUpdater,
          &SoftwareUpdateManager::userRequestedChangelog);
  connect(installButton, &QAbstractButton::clicked, swUpdater,
          &SoftwareUpdateManager::userRequestedInstall);

  connect(swUpdater, &SoftwareUpdateManager::requestGracefulShutdown, this,
          &VdcEditorWindow::close);
  connect(swUpdater, &SoftwareUpdateManager::updateAvailable,
          [this] { ui->updateBar->setVisible(true); });
...
void VdcEditorWindow::createRecentFileActions() {
  ...
  for (auto i = 0; i < maxFileNr; ++i) {
    ...
    QObject::connect(recentFileAction, &QAction::triggered, this, [this] {
      QAction *action = qobject_cast<QAction *>(sender());
      if (action) {
        VdcProjectManager::instance().closeProject();
        bool success = VdcProjectManager::instance().loadProject(
            action->data().toString());

        if (!success)
          QMessageBox::critical(this, "Error",
                                "No valid data found. The selected file does "
                                "not exist or contains unknown data.");
      }
    });
  ...
  auto *clearAct = new QAction(QIcon(QPixmap(":/img/clear_recents.svg")),
...
void VdcEditorWindow::loadProject() {
  QString file =
      QFileDialog::getOpenFileName(this, tr("Open VDC Project File"), "",
  ...
  if (!success)
    QMessageBox::critical(this, "Error",
                          "No valid data found. The selected file does not "
                          "exist or contains unknown data.");
...
void VdcEditorWindow::closeProject() {
  ...
  reply = QMessageBox::question(
      this, "DDC Toolbox",
      tr("Are you sure? All unsaved changes will be lost."),
...
void VdcEditorWindow::exportProject() {
  if (filterModel->rowCount() < 1) {
    QMessageBox::warning(
        this, tr("Error"),
        tr("Your current project is empty and contains no filters"));
    return;
  }
  ...
  if (sender() == ui->actionVDC) {
    ...
    QString fileName = QFileDialog::getSaveFileName(this, tr("Save VDC"), "",
                                                    tr("VDC File (*.vdc)"));
    ...
    if (srDlg->getResult().count() < 1) {
      QMessageBox::warning(this, "Error",
                           "Export cancelled due to empty selection");
      return;
    }
    ...
    /* Check whether a user attempts to export custom filters (which only
     * support 44100Hz and 48000Hz fields) using an unsupported srate */
      ...
      for (const auto &filter : qAsConst(filterModel->getFilterBank())) {
        if (filter->GetFilterType() == FilterType::CUSTOM) {
          auto button = QMessageBox::warning(
              this, "Export",
              "<b>Custom filters in your project can not be exported using the "
              "selected sampling rate</b><br/>"
              "Currently, custom filters can only contain filter coefficents "
              "designed for sampling rates 44100Hz and 48000Hz. Please choose "
              "a supported value to ensure all filters are exported "
              "correctly.<br/>"
              "Select 'Ignore' if you want to skip the affected filters and "
              "continue anyway.",
    ...
    QString fileName =
        QFileDialog::getSaveFileName(this, tr("Save EqualizerAPO config"), dir,
                                     tr("Equalizer APO config file (*.txt)"));
    ...
    QString fileName = QFileDialog::getSaveFileName(
        this, tr("Save CSV dataset"), "", tr("CSV dataset (*.csv)"));

    VdcProjectManager::instance().exportCsv(fileName, p, dlg->delimiter(),
                                            dlg->format(), dlg->numFormat(),
                                            dlg->includeHeader());
    dlg->deleteLater();
  }
}
...
void VdcEditorWindow::curveFitting() {
  ...
  if (ret && dlg->getResults().count() > 0) {
    ...
    QMessageBox::critical(this, "Error",
                          "Failed to process data. Algorithm returned an empty "
                          "response or was terminated.");
  }
...
void VdcEditorWindow::removePoint() {
  ...
  if (select->hasSelection()) {
    QUndoCommand *removeCommand =
        new RemoveCommand(filterModel, select->selectedRows());
...
void VdcEditorWindow::invertSelection() {
  ...
  if (!select->hasSelection()) {
    QMessageBox::information(this, tr("Invert selection"),
                             tr("No rows selected"));
    return;
  }

  QUndoCommand *invertCommand =
      new InvertCommand(filterModel, select->selectedRows());
...
void VdcEditorWindow::shiftSelection() {
  ...
  if (!select->hasSelection()) {
    QMessageBox::information(this, tr("Shift selection"),
                             tr("No rows selected"));
    return;
  }

  ShiftFrequencyDialog *sf =
      new ShiftFrequencyDialog(filterModel, select->selectedRows(), this);
...
void VdcEditorWindow::checkStability() {
  ...
  if (!s->hasIssue())
    QMessageBox::information(this, "Stability check",
                             "All filters appear to be stable");
  else
    s->exec();
  s->deleteLater();
}
...
void VdcEditorWindow::savePlotScreenshot() {
  ...
  if (obj == ui->actionScr_Magnitude_response)
    ui->graph->saveScreenshot();
  else if (obj == ui->actionScr_Phase_response)
    ui->phase_graph->saveScreenshot();
  else if (obj == ui->actionScr_Group_delay)
    ui->gdelay_graph->saveScreenshot();
}

void VdcEditorWindow::updatePlots(bool onlyUpdatePoints) {
  if (!onlyUpdatePoints) {
    const int bandCount = 8192 * 2;
    QVector<float> responseTable =
        filterModel->getMagnitudeResponseTable(bandCount, 48000.0);
    QVector<float> phaseTable =
        filterModel->getPhaseResponseTable(bandCount, 48000.0);
    QVector<float> gdelayTable =
        filterModel->getGroupDelayTable(bandCount, 48000.0);
    ui->graph->clearAll();
...
void VdcEditorWindow::showHelp() {
  ...
  if (sender() == nullptr) {
    return;
  } else if (sender() == ui->actionIntroduction)
    res = ":/html/introduction.html";
  else if (sender() == ui->actionKey_shortcuts)
    res = ":/html/keycombos.html";
...
void VdcEditorWindow::importClassicVdc() {
  ...
  if (!success)
    QMessageBox::critical(this, "Error",
                          "No valid data found\nKeep in mind that only classic "
                          "VDC files with 'Peaking filters' are supported.");
}

void VdcEditorWindow::importParametricAutoEQ() {
  QString file = QFileDialog::getOpenFileName(
      this, tr("Import AutoEQ config 'ParametricEQ.txt'"), "",
...
void VdcEditorWindow::downloadFromAutoEQ() {
  ...
  if (sel->exec() == QDialog::Accepted) {
    ...
    VdcProjectManager::instance().closeProject();
    bool success = VdcProjectManager::instance().loadParametricEqString(
        hp.getParametricEQ());
    if (!success)
      QMessageBox::critical(this, "Error",
                            "No valid data found\nConversion failed");
...
void VdcEditorWindow::batchConvert() {
  ...
  QString requestedType =
      isVdcConversion
          ? "VDC files (*.vdc)"
          : "AutoEQ ParametricEQ.txt (*ParametricEQ.txt);;All files (*.*)";

  QStringList files =
      QFileDialog::getOpenFileNames(this, tr("Select all files to convert"),
  ...
  if (!files.isEmpty()) {
    ...
    for (const auto &file : files) {
      ...
      VdcProjectManager::writeProject(
          QDir(dir).filePath(QFileInfo(file).completeBaseName() + ".vdcprj"),
          biquads);
    }
...
void VdcEditorWindow::closeEvent(QCloseEvent *ev) {
  if (VdcProjectManager::instance().hasUnsavedChanges() &&
    ...
    int ret = QMessageBox::warning(this, tr("DDCToolbox"),
  ...
  if (VdcProjectManager::instance().hasUnsavedChanges() &&
    ...
    if (manager.allowsInteraction()) {
      int ret = QMessageBox::warning(
          this, tr("DDCToolbox"),
          tr("The document has been modified and your operating system has "
