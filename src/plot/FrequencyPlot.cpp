...
void FrequencyPlot::setMode(PlotType type, QWidget *parent) {
  ...
  connect(this, &QCustomPlot::mouseMove, this, [this](QMouseEvent *event) {
    // Use Qt6 API: QMouseEvent::position() returns QPointF. Convert to the same
    // coordinate space.
    int x = (int)xAxis->pixelToCoord(event->position().x());
    float y = (float)yAxis->pixelToCoord(event->position().y());
    if (x < 0 || x > 24000)
      return;
  ...
  connect(this, &QCustomPlot::customContextMenuRequested, this,
...
void FrequencyPlot::setAllMarkerPointsVisible(bool b) { m_markerVisible = b; }
...
void FrequencyPlot::updatePoints() {
  ...
  for (const auto &index : host->selectionModel()->selectedRows(0)) {
    if (model->getFilter(index.row())->GetFilterType() ==
            FilterType::UNITY_GAIN ||
